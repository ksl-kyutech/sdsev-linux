...
u64 start;
u64 finish;
u64 rmpupdate_time=0;
u64 rmpupdate_num = 0;
// SBI CALL
#define SBI_EXT_SD_SEV 0x08000000
#define STATE_TO_HYPERVIOSR_RMPUPDATE 0x0a
	...
	if (vring_use_dma_api(vdev)) {
		...
		if (queue) {
			...
			*dma_handle = (dma_addr_t)phys_addr;
			...
			//ハイパーバイザがアクセスできるようにRMPを変更
			int page_num = PAGE_ALIGN(size)/4096;
			unsigned long gpa;
			//pr_notice("GPA: 0x%lx size: 0x%lx\n",phys_addr, PAGE_ALIGN(size) );
			for(int i=0; i < page_num; i++){
				gpa =  phys_addr +  i*4096;
				struct sbiret ret = sbi_ecall(SBI_EXT_SD_SEV, STATE_TO_HYPERVIOSR_RMPUPDATE, gpa,0, 0, 0, 0, 0);
				if(ret.error){
					pr_notice("RMPUPDATE Failed\n"); //add
				}
			}
			...
			rmpupdate_num++;
			finish = ktime_get_ns();
			rmpupdate_time += (finish-start);
			pr_notice("\nRMPUPDATE: %ld num: %ld\n", rmpupdate_time,rmpupdate_num);
				      ...
				      gfp_t gfp)
{
	...
	for (n = 0; n < out_sgs; n++) {
		for (sg = sgs[n]; sg; sg = sg_next(sg)) {
			...
			start = ktime_get_ns();
			//ハイパーバイザがアクセスできるようにRMPを変更
			int page_num = PAGE_ALIGN(len)/4096;
			unsigned long gpa;
			//pr_notice("GPA: 0x%lx size: 0x%lx\n",addr, PAGE_ALIGN(len) );
			for(int i=0; i < page_num; i++){
				gpa =  addr +  i*4096;
...
bool virtqueue_notify(struct virtqueue *_vq)
{
	...
	//pr_notice("end virtqueue_notify\n");
	return true;
}
